...
#ifndef STL_MALLAREGULAR_H
#define STL_MALLAREGULAR_H
#include <vector>
#include <list>
#include <cmath>
#include "Casilla.h"
...
T MallaRegular<T>::buscar(float x, float y, const T &dato) {
    if(x<xMin || y<yMin || x>xMax || y>yMax){
        throw std::out_of_range ("Lo siento ese rango se encuentra fuera de la malla");
    }
    Casilla<T> c = obtenerCasilla(x,y);
...
std::vector<T> MallaRegular<T>::buscarRadio(float xcentro, float ycentro, float radio) {
    ...
    if(xcentro > xMax || xcentro < xMin || ycentro < xMin || ycentro > yMax){
    ...
    while(x_aux<xcentro+radio){ //esquina sup derecha
        ...
        while (y_aux<ycentro+radio){ //esquina sup izquierda
            ...
            for (auto itrecorre = c->puntos.begin();itrecorre != c->puntos.end(); ++itrecorre){
                float d = calcularDistancia(xcentro,itrecorre->getUTM.getLatitud,ycentro,itrecorre->getUTM.getLatitud);
                if(d < radio) {
